...
const ShowJobExperience = ({setOpenJobForm}) => {
   ...
   const {student} =useSelector((state)=>state.StudentReducer);
    const SingleJobValue=(id,comapanyname,position,description,startdate,enddate)=>{
        ...
        setNewSelectedStartDate(convertDate);
        setNewSelectedEndDate(convertEndDate);
    ...
    const UpdateSingleJob= async ()=>{  
      ...
      try {
         const {data}=await instance.post(`/resume/edit-jobs/${updatedId}`,jobEditData);
     ...
     <Fragment>
         ...
         <StudentJobExperienceWrapper_NavPart>
             <StudentJobExperienceWrapper_NavPart_Section1>
              ...
              <AddButtonStyler onClick={()=>setOpenJobForm(true)}><AddOutlined/>Add Job Exp</AddButtonStyler>
         </StudentJobExperienceWrapper_NavPart>
         ...
         <DynamicJobWrapper key={dets.id}>
              ...
              <DynamicDescription>
                 <DynamicDescriptionStyler>
                    <AccordionSummary expandIcon={<ExpandMoreOutlined/>}>
                        <DynamicSummaryStyler>Description Of Position</DynamicSummaryStyler>
                    </AccordionSummary>
                    <AccordionDetails>
           ...
           <DynamicJobEditButton onClick={()=>SingleJobValue(dets.id,dets.comapanyName,dets.position,dets.description,dets.startDate.substring(0,10),dets.endDate.substring(0,10))} ><EditOutlined/> &nbsp; Edit</DynamicJobEditButton>
         </DynamicJobWrapper>
          ))}
        </StudentJobExperienceWrapper>
         {editSingleJob? <Dialog open={editSingleJob} PaperProps={{sx:dialogStyler}}>
          <JobDialogWrapper>
              ...
              <JobDialogWrapper_form>
                  <JobDialogWrapper_Content>
                      ...
                      <JobDialogWrapperInput placeholder='Update the name' value={updatedComapnyName} onChange={(e)=>setUpdatedCompanyName(e.target.value)}/>
                  </JobDialogWrapper_Content>
                  <JobDialogWrapper_Content>
                      <JobDialogWrapperHeading>Position Name</JobDialogWrapperHeading>
                      <JobDialogWrapperInput placeholder='Type Position' value={updatedPosition} onChange={(e)=>setUpdatedPosition(e.target.value)}/>
                  </JobDialogWrapper_Content>
                  <JobDialogWrapper_Content>
                      <JobDialogWrapperHeading>Update the Description</JobDialogWrapperHeading>
                      <JobDialogWrapperInput placeholder='Type Description' value={updatedDescription} onChange={(e)=>setUpdatedDescription(e.target.value)}/>
                  </JobDialogWrapper_Content>
                  <JobDialogWrapper_DateWrapper>
                       <JobDialogWrapper_StartEndWrapper>
                          <Typography style={{fontSize:"18px",opacity:"0.5",fontWeight:"700"}} >Set the New Date</Typography>
                          <DatePicker
              ...
              <JobDialogButtonWrapper>
                 <DeleteButtonStyler onClick={()=>DeleteSingleJob()}>Delete Job</DeleteButtonStyler>
                 <EditWrapperSection2ButtonWrapper>
                     <EditWrapperCancelButton onClick={()=>setEditSingleJob(false)}>Cancel</EditWrapperCancelButton>
                     <EditWrapperSaveButton onClick={()=>UpdateSingleJob()}>Update Details</EditWrapperSaveButton >
                 </EditWrapperSection2ButtonWrapper>
              </JobDialogButtonWrapper>
          </JobDialogWrapper>
       </Dialog>:""}
